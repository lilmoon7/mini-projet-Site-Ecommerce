...
export class ProductService {
  ...
  private extractNumericPrice(product: Product): number {
    const priceStr = product.prouctPrice.toString().replace(/[^\d.]/g, '');
    return parseFloat(priceStr) || 0;
  }
  filterByPriceRange(minPrice: number, maxPrice: number): Product[] {
    return this.products.filter(p => {
      const price = this.extractNumericPrice(p);
      return price >= minPrice && price <= maxPrice;
    });
  }
  getMostExpensiveProducts(limit: number = 5): Product[] {
    return [...this.products]
      .sort((a, b) => this.extractNumericPrice(b) - this.extractNumericPrice(a))
      .slice(0, limit);
  }
  getCheapestProducts(limit: number = 5): Product[] {
    return [...this.products]
      .sort((a, b) => this.extractNumericPrice(a) - this.extractNumericPrice(b))
      .slice(0, limit);
  }
  ...
  updateProduct(productId: String, updatedProduct: Product): boolean {
    const index = this.products.findIndex(p => p.productID === productId);
    if (index !== -1) {
      const originalId = this.products[index].productID;
      this.products[index] = updatedProduct;
      if (this.products[index].productID !== originalId) {
        const correctedProduct = new Product(originalId);
        correctedProduct.productTitle = updatedProduct.productTitle;
        correctedProduct.prouctPrice = updatedProduct.prouctPrice;
        correctedProduct.productImage = updatedProduct.productImage;
        correctedProduct.category = updatedProduct.category;
        this.products[index] = correctedProduct;
      }
      return true;
    }
    return false;
  ...
  printAllProducts(): void {
    ...
    console.log(`Total: ${this.products.length} produits`);
  }

  getTotalValue(): number {
    return this.products.reduce((total, product) => {
      return total + this.extractNumericPrice(product);
    }, 0);
  }
  ...
  createProduct(
    ...
    product.prouctPrice = price.includes('DH') ? price : `${price} DH`;
